...
var modals = function modals() {
  ...
  var scrollLineWidth = calcScrollLine();

  function bindModal(triggerSelector, modalSelector, closeSelector) {
    var overlayClickClose = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : true;
  ...
  function calcScrollLine() {
    ...
    var scrollLineWidth = div.offsetWidth - div.clientWidth;
    div.remove();
  ...
  function showPopupByTime(modalSelector, ms) {
    ...
    setTimeout(function () {
      ...
      if (!isOpen) {
        ...
        var _scrollLineWidth = calcScrollLine();

        document.body.style.marginRight = _scrollLineWidth + 'px';
      }
    }, ms);
  }
  ...
  function openByScrollAtBottom(triggerSelector) {
    ...
    window.addEventListener('scroll', function () {
      var scrolledAndCurrContent = window.scrollY + document.documentElement.clientHeight; // window.scrollY - сколько пкс юзер отлистал сверху
      // document.documentElement.clientHeight - контент кот виден юзеру

      var fullPageHeight = Math.max(document.documentElement.scrollHeight, document.body.scrollHeight); // для совместимости
      // document.documentElement.scrollHeight - полная высота страницы

      if (!isBtnPressed && scrolledAndCurrContent >= fullPageHeight) {
        trigger.click(); // window.onscroll = null
      }
    });
  }
  ...
  openByScrollAtBottom('.fixed-gift'); // showPopupByTime('.popup-consultation', 2000)
};
